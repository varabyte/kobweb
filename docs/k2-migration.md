...
- *(Recommended)* Removes the Jetbrains Compose Gradle plugin from every Kobweb module & replaces compose dependency
  ...
  with new version catalog entries
    - Now that the Compose compiler is applied with its own Gradle plugin, there is no requirement to use the
      ...
      ```
- *(Required)* Updates any `.kobweb/conf.yaml` files to reference the new Kotlin compilation directory, which changed
  from `dist`
  `kotlin-webpack`:
  ```diff
  dev:
...
- (**TODO: NOT IMPLEMENTED YET - EITHER IMPLEMENT OR REMOVE**) If you have full-stack site, you can now move
  the `compose-runtime` dependency from `commonMain` to `jsMain`. This is
  possible because Kobweb now configures the compose compiler to run only on the JS target.
