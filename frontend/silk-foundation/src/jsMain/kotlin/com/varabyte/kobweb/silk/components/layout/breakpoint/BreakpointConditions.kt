...
@Deprecated("Update the import to use `com.varabyte.kobweb.silk.breakpoint.displayIfAtLeast` instead.")
fun Modifier.displayIfAtLeast(breakpoint: Breakpoint) =
    this.classNames("silk-display-if-at-least-${breakpoint.name.lowercase()}")

@Deprecated("Update the import to use `com.varabyte.kobweb.silk.breakpoint.displayUntil` instead.")
fun Modifier.displayUntil(breakpoint: Breakpoint) = this.classNames("silk-display-until-${breakpoint.name.lowercase()}")

@Deprecated("Update the import to use `com.varabyte.kobweb.silk.breakpoint.displayBetween` instead.")
