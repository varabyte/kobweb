...
class Router {
    ...
    fun registerRedirect(fromRoute: String, toRoute: String, autoPrefix: Boolean = true) {
        ...
        require(
            routeTree.registerRedirect(
                RoutePrefix.prependIf(autoPrefix, fromRoute),
                RoutePrefix.prependIf(autoPrefix, toRoute)
            )
        ) {
            "Redirect registration failure. `$fromRoute` cannot be bound to `$toRoute`. Was it actually registered?"
        }
    }
