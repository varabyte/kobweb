...
abstract class KobwebMigrateToK2Task :
    ...
    @TaskAction
    fun execute() {
        // NOTE: This project is only registered to the root project
        ...
        versionsFile.takeIf { project == project.rootProject }?.let { file ->
            ...
            val updatedText = originalText
                ...
                .replace("kotlin = \"1.9.2[0-4]\"".toRegex(), "kotlin = \"2.0.0\"")
                .replace("jetbrains-compose = \"1.6.[0-2]\"".toRegex(), "jetbrains-compose = \"1.6.10\"")
                .replace(
        ...
        project.subprojects {
            ...
            pluginManager.withPlugin("com.varabyte.kobweb.application") {
                buildScripts.addAll(subProject.layout.getBuildScripts().map { it.toPath() })
