...
abstract class KobwebCopyWorkerJsOutputTask @Inject constructor(private val appBlock: AppBlock) : KobwebCopyTask(
    ...
    private data class CopiedWorkerFile(
    ...
    fun execute() {
        ...
        val copiedFiles = mutableMapOf<String, CopiedWorkerFile>()
        val workerOutputData = runtimeClasspath.toKobwebOutputByPattern(objectFactory, archiveOperations, workerOutputsFilesPattern)
            ...
            workerOutputData.forEach { (jar, rootAndFile) ->
                from(rootAndFile.file) {
                    ...
                    val targetFile = getGenPublicRoot()
                        ...
                        .resolve(relativeWorkerPath)
                    val copiedWorkerFile = CopiedWorkerFile(
                        sourceJar = jar.name,
                        relativePath = relativeWorkerPath,
                        contentHash = rootAndFile.file.inputStream().use { inputStream ->
                    ...
                    copiedFiles.putIfAbsent(targetFile.absolutePath, copiedWorkerFile)?.let { existing ->
                        if (existing == copiedWorkerFile) return@from
                        throw GradleException("You are attempting to use two different worker dependencies that both have the same group and worker name: ${existing.sourceJar} and ${jar.name}. They are incompatible and you cannot use both. If you own the project for either, consider changing the Gradle group or the worker name (via `configAsKobwebWorker(...)`) on one or the other to avoid this naming collision.")
                    }
