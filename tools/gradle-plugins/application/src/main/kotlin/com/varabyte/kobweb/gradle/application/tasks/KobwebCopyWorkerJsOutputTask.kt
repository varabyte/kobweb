...
abstract class KobwebCopyWorkerJsOutputTask @Inject constructor(kobwebBlock: KobwebBlock) : KobwebModuleTask(
    kobwebBlock, "Copy any JS output files from any Kobweb worker dependencies and copy them to the final site's resources"
) {
    ...
    fun execute() {
        ...
        classpath
            .flatMap { jar ->
                if (jar.isDirectory) {
                    project.fileTree(jar).toWorkerOutputResources(jar)
                } else {
                    try {
                        project.zipTree(jar).toWorkerOutputResources(jar)
                    } catch (ex: Exception) {
