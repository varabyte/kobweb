        ...
        try {
            for (frame in incoming) {
                if (frame is Frame.Text) {
                    ...
                    try {
                        when (val payload = incomingMessage.payload as StreamMessage.Payload.Client) {
                            StreamMessage.Payload.Client.Connect -> {
                            ...
                            StreamMessage.Payload.Client.Disconnect -> streamImpl.disconnect()
                        ...
                        val callstack =
                            if (env == ServerEnvironment.DEV) t.stackTraceToString(includeUntil = {
                                it.className == Apis::class.qualifiedName || it.className.startsWith(
                                    ApiStream::class.qualifiedName!!
                                )
                            })
                            else null
